...
Daremos como ejemplo dos esquemas clásicos de montaje del centro. Ambos son totalmente válidos y funcionan perfectamente.

:::info
La mayoría de capturas de pantalla están en inglés, puesto que no tengo la costumbre de cambiar el lenguaje original de los programas. Obviamente si alguien quiere configurar los parámetros en la ventana de login al catalán o al español lo puede hacer sin problemas.
:::

## Esquema 1

En este esquema tenemos como ejemplo dos switchs con 3 LAG configurados al switch principal por sí queremos montar un cluster con PROXMOX con hasta 3 ordenadores. Son 3 ordenadores ya que 3 ordenadores permiten hacer un montaje de Alta disponibilidad, pero esto es un tema que explicaremos en cursos más avanzados. Este esquema es típico de muchos centros grandes, que tienen varías salas de informática y el switch principal prácticamente lo utilizan para redistribuir las VLANs y que solo tienen montados LAGs que van a cada uno de los switch del centro.

:::info
 La alta disponibilidad (HA) permite que cuando uno hipervisor se estropea los otros cogen las máquinas virtuales del mismo y sigue dando servicio sin que el usuario lo note. Así permite cambiar el hipervisor o arreglarlo y el servicio no es interrumpido en ningún momento. Para hacer esto es necesario montar un CEPH o tener una cabina externa con mucha fiabilidad..
:::
...
Una de las principales ventajas del siguiente esquema es que no utiliza VLANs, cada tarjeta del hipervisor va a un switch diferente. Este montaje se utiliza principalmente en centro pequeños. Igualmente permite el montaje de un cluster que podríamos conectar al router del centro en las bocas de la red VLAN. El equema seria de la siguiente manera:

![Esquema orientativo 2](Esquemes/esquemasense.png)

## Esquema de máquinas virtuales

Ambos esquemas comparten el montaje de las máquinas virtuales puesto que la configuración de donde se conectan las tarjetas virtuales se hace desde el PROXMOX. Hemos respetado los nombres de las tarjetas en ambos casos (**vmbrX**), pero se puede dar el nombre que quieras.

![Esquema conexiones máquinas virtuales](Esquemes/Conexions hipervisor.png)

# Configuración del PROXMOX

Una vez tenemos instalado EL PROXMOX y hayaMOS reiniciado, podremos acceder en él. Toda la configuración del PROXMOX se realiza a través de un servidor web que lleva el sistema. Para acceder tenemos que hacerlo a través del puerto 8006 con certificación ssl. Sencillamente escribimos en un navegador de una estación de trabajo que esté en la misma red lo siguiente:
...
De manera similar, si queremos seguir todo el proceso habría que instalar los otros dos servidores con el mismo procedimiento. Lo único que hay que cambiar entre ellos sería el nombre de cada uno de ellos, nosotros hemos escogido la siguiente nomenclatura:
...
Una vez tenemos instalados todos los servidores procedemos a configurar la red. Para acceder a la configuración de la hipervisor tenemos que seleccionar el mismo (No la máquina virtual ni el Datacenter), y vamos a las opciones **Network**.
...
:::warning
Si decidís montar el /net a una cabina externa. Tened cuidado con las ACL!!!. Puesto que suelen dar problemas. La opción en este caso más segura para que funciono es tener el /net en un disco duro virtual. Además, si vais a tener espacio almacenado a una cabina, la cabina tiene que tener unas características adecuadas. Se recomendaría como mínimo:
...
:::info
En este curso no explicaremos como configurar una cabina para que funcione en al modelo de red. Las cabinas no entran dentro de la dotación, por lo tanto sería una adquisición propia del centro.
:::
...
En principio no sería necesario crear un backup de las máquinas virtuales puesto que hemos montado un RAID1 en el tema anterior, pero es muy común montar el PROXMOX en un disco y usar el otro para backups. Para configurar la copia de seguridad seleccionamos el datacenter, vamos a la opción de Backup y hacemos click sobre Add. Hemos d etener en cuenta que rescatar los datos en un RAID1 puede resultar bastante tedioso. Teniendo la copia de seguridad en una cabina o en el otro disco, el rescate puede resultar más sencillo.

![Configuración de cabina](ConProxmox/prox39.png)

Nos aparecerá la siguiente ventana, tenemos que tener en cuenta en qué lugar queremos hacer el backup (**Storage*). Y seleccionamos la/las máquinas virtuales que queremos hacer copia de sseguridad. En un principio, en nuestro caso solo habría que hacer la copia de seguridad del Máster, puesto que es donde se guarda el /net y LDAP. La copia de seguridad queda programada para una fecha determinada, en principio, en un centro podría ser un sábado a las 12 de la noche ya que no hay ningún usuario conectado.
